...
var Mom = window.Mom || (function() {
    ...
    //拼接参数到url中
    var extractUrl = function(url, params){
        var paramArr = typeof(params) == "string" ? [params] : params;
        for(var i=0; i<paramArr.length; i++){
            url+=(url.indexOf('?')>-1?'&':'?')+paramArr[i];
        }
        return url;
    };
    /**
    ...
    var layMsg = function(info, icon, time){
        ...
        p_.layer.msg(info, {icon:icon, time:time, shade:0.01});
    };
    //弹出警告信息
