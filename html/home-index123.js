...
function getSizeStyle(sizex,sizey,titleH){
    var titleH = titleH||0;
        ...
        ajaxToServer1_quite(url,{},tile,function(result){
            if(result.success=true){
                if (tile.templateCode == 'line') {
                    ec.render.line(result.rows, tile.tileTemplateId, tile);
                } else if (tile.templateCode == 'bar') {
                    ec.render.bar(result.rows, tile.tileTemplateId, tile);
                } else if (tile.templateCode == 'pie') {
                    ec.render.pie(result.rows, tile.tileTemplateId, tile);
                } else if (tile.templateCode == 'gauge') {
                    ec.render.gauge(result.rows, tile.tileTemplateId, tile);
                }
            }else{
...
function aClick(a, url){
    ...
    var token = getCookie("token_type") +' '+ getCookie("authorization");
    ...
    if(target == 1){  //判断a标签的打开形式  0 创建新的iframe 1  新窗口  2  layer形式
        ...
        if (top != self) {
            ...
            tab.url  = extractUrl(tab.url,"__Token="+encodeURI(token));
...
//面板中已有，磁贴商店中不能移动的磁贴
function store_tiles_unmove(tileTemplateId) {
...
function appsInit() {    //页面每次加载的时候都会调用该方法 然后获取到第一个下拉框的数据 将数据动态的渲染到#apps中
    app_array = [];
    ajaxToServer1("/api/workbench/WorkbenchHomePage/appAndTypeList",{},function(result){
        if(result.success){
